...
#[derive(Clone)]
pub struct Lexer {
    input: String,                  // Input string (owned, UTF-8)
    chars: Vec<char>,               // Input as Unicode scalar values (safe indexing)
    position: usize,                // Current position in `chars`
...
impl Lexer {
        ...
        match self.current_char {
            ...
            Some('=') => {
                ...
                if self.current_char == Some('=') {
                    ...
                    let lane_name = self.read_line_text();
                    Ok(Token::Lane(lane_name))
                } else {
                    let pool_name = self.read_line_text();
                    Ok(Token::Pool(pool_name))
                }
            },
            Some('#') => {
                self.advance(); // Skip '#'
                let text: String = self.read_line_text(); // Read the text after the event symbol
            ...
            Some('-') => {
                ...
                if self.current_char == Some('>') {
                    self.advance(); // Skip '>'
                    let label: String = self.read_identifier();
                    // Allow optional whitespace before quoted edge text.
            ...
            Some('J') => {
                self.advance(); // Skip 'J'
                let label: String = self.read_identifier(); // Read the label after 'J'
                // Allow optional whitespace before quoted join text.
                self.skip_whitespace();
                let text = self.read_quoted_text();
                Ok(Token::Join(label, text?))
            },
            Some(c) if !c.is_whitespace() => {
                let mut text = self.read_identifier_or_label();
                if text.ends_with(":") {
    ...
    /// Read the remainder of the current line (used for task/event/pool/lane names).
    /// Stops at newline or a quote (so quoted edge labels can be read separately).
    ...
    /// Read a single identifier token (labels after `->`, `<-`, `J`, and `label:` lines).
    /// Stops at whitespace, newline, quote, or ':'.
    ...
    /// Read either `label:` or a single identifier-like token.
    fn read_identifier_or_label(&mut self) -> String {
        let mut text = String::new();
        self.skip_whitespace();
        while let Some(c) = self.current_char {
            if c.is_whitespace() || c == '\n' || c == '"' {
    ...
    fn read_quoted_text(&mut self) -> Result<String, LexerError> {
        // Quotes often come after whitespace (e.g. `->label "text"`).
        self.skip_whitespace();
        if self.current_char == Some('"') {
            ...
            while let Some(c) = self.current_char {
                ...
                // Allow anything (including '-') inside quoted text.
                if c == '\n' {
    ...
    pub fn highlight_error(&self) -> String {
        ...
        let pad = self.column.saturating_sub(2);
        let highlight = " ".repeat(pad) + "^";
